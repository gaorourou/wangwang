<template>
  <div id="detailContrainer" class="slide-enter-active">
    <Header title="影片详情" slot="">
      ...
      <div class="detail_list">
				<div class="detail_list_bg" :style="{ 'background-image' : 'url('+ detailMovie.images.large+')' }"></div>
				<div class="detail_list_filter"></div>
				<div class="detail_list_content">
					<div class="detail_list_img">
						<img :src="detailMovie.images.small" alt="">
					</div>
					<div class="detail_list_info">
						<h2>{{detailMovie.title}}</h2>
						<p>{{detailMovie.original_title}}</p>
						<p>{{detailMovie.rating.average}}</p>
						<p>{{(detailMovie.genres).toString()}}</p>
						<p>中国大陆 / {{(detailMovie.durations[0]).toString()}}</p>
						<p>{{detailMovie.mainland_pubdate}}大陆上映</p>
					</div>
				</div>
			</div>
      <div class="detail_intro">
...
export default {
  ...
  mounted() {
    ...
    this.axios.get('https://douban.uieee.com/v2/movie/subject/'+this.movieId)
    .then((res)=>{
      console.log(res);
...
#content .detail_intro>p{display: -webkit-box;-webkit-box-orient: vertical;-webkit-line-clamp: 6;overflow:hidden;}
